...
<body>
    ...
    <form method="get" action="">
        ...
        <div id="form-tahun">
            ...
            <select name="tahun">
                <option value="">Pilih</option>
                <?php
                $query = "SELECT YEAR(tgl_daftar) AS tahun FROM daftar GROUP BY YEAR(tgl_daftar)"; // Tampilkan tahun sesuai di tabel transaksi
    ...
    if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
        ...
        if($filter == '1'){ // Jika filter nya 1 (per tanggal)
            ...
            $query = "SELECT * FROM daftar WHERE DATE(tgl_daftar)='".$_GET['tanggal']."'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
            ...
            $query = "SELECT * FROM daftar WHERE MONTH(tgl_daftar)='".$_GET['bulan']."' AND YEAR(tgl_daftar)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
        }else{ // Jika filter nya 3 (per tahun)
            echo '<b>Data Transaksi Tahun '.$_GET['tahun'].'</b><br /><br />';
            echo '<a href="print.php?filter=3&tahun='.$_GET['tahun'].'">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM daftar WHERE YEAR(tgl_daftar)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
        }
    }else{ // Jika user tidak mengklik tombol tampilkan
        echo '<b>Semua Data Transaksi</b><br /><br />';
        echo '<a href="print.php">Cetak PDF</a><br /><br />';

        $query = "SELECT * FROM daftar ORDER BY tgl_daftar"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
    }
    ?>
